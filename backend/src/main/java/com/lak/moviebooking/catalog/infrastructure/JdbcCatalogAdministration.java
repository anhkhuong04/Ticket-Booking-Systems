...
class JdbcCatalogAdministration implements CatalogAdministration {
    ...
    private static final Set<String> MOVIE_STATUSES = Set.of("NOW_SHOWING", "COMING_SOON", "ARCHIVED");
    private final JdbcTemplate jdbcTemplate;
    private final AuditLogWriter auditLogWriter;
    private final Clock clock;

    JdbcCatalogAdministration(JdbcTemplate jdbcTemplate, AuditLogWriter auditLogWriter, Clock clock) {
    ...
    public MovieDetail createMovie(UUID actorId, MovieWriteCommand command) {
        ...
        jdbcTemplate.update("""
                INSERT INTO movies (id, title, description, duration_minutes, age_rating, release_date, poster_url, trailer_url, status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, movieId, command.title().trim(), nullable(command.description()), command.durationMinutes(), command.ageRating().trim(),
                command.releaseDate(), nullable(command.posterUrl()), nullable(command.trailerUrl()), command.status(), now, now);
    ...
    public MovieDetail updateMovie(UUID actorId, UUID movieId, MovieWriteCommand command) {
        ...
        int changed = jdbcTemplate.update("""
                UPDATE movies SET title = ?, description = ?, duration_minutes = ?, age_rating = ?, release_date = ?,
                    poster_url = ?, trailer_url = ?, status = ?, updated_at = ? WHERE id = ?
                """, command.title().trim(), nullable(command.description()), command.durationMinutes(), command.ageRating().trim(),
                command.releaseDate(), nullable(command.posterUrl()), nullable(command.trailerUrl()), command.status(), now(), movieId);
        if (changed != 1) {
    ...
    private MovieDetail findMovie(UUID movieId) {
        return jdbcTemplate.query("""
                ...
                List.of((String[]) rs.getArray("genres").getArray())), movieId).stream().findFirst()
