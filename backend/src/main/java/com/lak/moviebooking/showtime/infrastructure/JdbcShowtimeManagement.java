...
class JdbcShowtimeManagement implements ShowtimeManagement {
    ...
    JdbcShowtimeManagement(JdbcTemplate jdbcTemplate, MovieCatalogQuery movieCatalogQuery, CinemaShowtimeQuery cinemaShowtimeQuery, AuditLogWriter auditLogWriter, ShowtimeCancellationAudit showtimeCancellationAudit, Clock clock) { this.jdbcTemplate=jdbcTemplate; this.movieCatalogQuery=movieCatalogQuery; this.cinemaShowtimeQuery=cinemaShowtimeQuery; this.auditLogWriter=auditLogWriter; this.showtimeCancellationAudit=showtimeCancellationAudit; this.clock=clock; }
    ...
    @Override @Transactional public AdminShowtimeView cancelShowtime(UUID actorId, UUID showtimeId) {
        AdminShowtimeView showtime=jdbcTemplate.query("SELECT s.id,c.id cinema_id,c.name cinema_name,a.id auditorium_id,a.name auditorium_name,m.id movie_id,m.title movie_title,s.start_at,s.end_at,s.sales_close_at,s.status,EXISTS (SELECT 1 FROM showtime_seats ss WHERE ss.showtime_id=s.id AND ss.status IN ('HELD','SOLD','PAYMENT_PENDING')) cancellation_blocked FROM showtimes s JOIN auditoriums a ON a.id=s.auditorium_id JOIN cinemas c ON c.id=a.cinema_id JOIN movies m ON m.id=s.movie_id WHERE s.id=? FOR UPDATE",(rs,row)->adminShowtime(rs),showtimeId).stream().findFirst().orElseThrow(()->ApplicationException.notFound("SHOWTIME_NOT_FOUND","Showtime was not found"));showtimeCancellationAudit.recordRequested(actorId,showtimeId);if(!"SCHEDULED".equals(showtime.status()))throw ApplicationException.businessRule("SHOWTIME_NOT_SCHEDULED","Only scheduled showtimes can be cancelled");if(showtime.cancellationBlocked())throw ApplicationException.businessRule("SHOWTIME_CANCELLATION_REQUIRES_REFUND","Showtime has active holds, sold seats, or payment-pending seats and cannot be cancelled safely");jdbcTemplate.update("UPDATE showtimes SET status='CANCELLED',updated_at=? WHERE id=?",now(),showtimeId);auditLogWriter.record(actorId,"SHOWTIME_CANCELLED","showtime",showtimeId,Map.of("reason","NO_ACTIVE_CUSTOMER_SEATS"));return new AdminShowtimeView(showtime.id(),showtime.cinemaId(),showtime.cinemaName(),showtime.auditoriumId(),showtime.auditoriumName(),showtime.movieId(),showtime.movieTitle(),showtime.startAt(),showtime.endAt(),showtime.salesCloseAt(),"CANCELLED",false);
    }
