    ...
    <section className="mt-6 overflow-x-auto rounded-xl border border-border bg-surface">
      <table className="min-w-full text-left text-sm">
        ...
        <tbody>{filteredMovies.map((movie) => <tr key={movie.id} className="border-b border-border">
          <td className="p-3"><div className="flex min-w-52 items-center justify-between gap-3"><div className="min-w-0 font-medium"><span className="block truncate">{movie.title}</span><span className="block text-xs font-normal text-text-muted">{movie.durationMinutes} phút · {movie.genres.join(', ')}</span></div>{movie.posterUrl ? <img src={movie.posterUrl} alt={`Poster phim ${movie.title}`} className="h-16 w-11 shrink-0 rounded-md object-cover" loading="lazy" /> : <span className="grid h-16 w-11 shrink-0 place-items-center rounded-md bg-slate-100 px-1 text-center text-[10px] font-medium text-text-muted">Chưa có poster</span>}</div></td>
