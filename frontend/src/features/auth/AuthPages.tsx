...
export function LoginPage() {
  const { login } = useAuth(); const navigate = useNavigate(); const location = useLocation(); const [error, setError] = useState<string | null>(null); const [pending, setPending] = useState(false)
  ...
  return <AuthCard title="Đăng nhập"><form className="mt-6 space-y-4" onSubmit={submit}><ErrorMessage value={error}/><Field label="Email" name="email" type="email" autoComplete="email" required/><Field label="Mật khẩu" name="password" type="password" autoComplete="current-password" required/><Submit pending={pending}>Đăng nhập</Submit></form><div className="mt-5 flex justify-between text-sm"><Link className="text-primary underline" to="/forgot-password">Quên mật khẩu?</Link><Link className="text-primary underline" to="/register">Đăng ký</Link></div></AuthCard>
}
