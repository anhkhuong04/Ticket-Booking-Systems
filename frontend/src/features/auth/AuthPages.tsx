...
export function LoginPage() {
  const { login } = useAuth(); const navigate = useNavigate(); const location = useLocation(); const [error, setError] = useState<string | null>(null); const [pending, setPending] = useState(false); const [showPassword, setShowPassword] = useState(false); const [rememberMe, setRememberMe] = useState(true)
  ...
  return <AuthCard title="Đăng nhập"><form autoComplete="on" className="mt-7 space-y-5" onSubmit={submit}><ErrorMessage value={error}/><LoginField label="Email" name="email" type="email" placeholder="Nhập email của bạn" icon={Mail}/><LoginField label="Mật khẩu" name="password" type={showPassword ? 'text' : 'password'} placeholder="Nhập mật khẩu" icon={LockKeyhole} visible={showPassword} onToggle={() => setShowPassword((value) => !value)}/><div className="flex items-center justify-between gap-3 text-sm"><label className="inline-flex min-h-11 cursor-pointer items-center gap-2 font-medium text-text-secondary"><input type="checkbox" name="rememberMe" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} className="peer sr-only" /><span className="grid size-5 place-items-center rounded-md border border-slate-300 bg-white text-transparent peer-checked:border-primary peer-checked:bg-primary peer-checked:text-white peer-focus-visible:outline-2 peer-focus-visible:outline-info"><Check size={15} strokeWidth={3} aria-hidden="true" /></span>Ghi nhớ tôi</label><Link className="font-semibold text-primary underline underline-offset-2" to="/forgot-password">Quên mật khẩu?</Link></div><button type="submit" disabled={pending} className="flex min-h-12 w-full items-center justify-center gap-3 rounded-lg bg-primary px-4 py-2 font-semibold text-white transition-colors hover:bg-primary-hover disabled:opacity-60"><span>{pending ? 'Đang xử lý…' : 'Đăng nhập'}</span><ArrowRight size={21} aria-hidden="true" /></button><div className="flex items-center gap-4 py-1 text-sm text-text-muted"><span className="h-px flex-1 bg-border" /> <span>Hoặc</span> <span className="h-px flex-1 bg-border" /></div><p className="text-center text-sm font-medium text-text-secondary">Chưa có tài khoản? <Link className="font-bold text-primary underline underline-offset-2" to="/register">Đăng ký</Link></p><p className="pt-2 text-center text-xs leading-5 text-text-muted">Bằng việc đăng nhập, bạn đồng ý với <Link className="underline" to="/terms">Điều khoản sử dụng</Link><br/>và <Link className="underline" to="/privacy">Chính sách bảo mật của LAK Cinema</Link>.</p></form></AuthCard>
}
