...
export function CheckoutPage() {
  ...
  return <main className="min-h-screen bg-background pb-28 pt-6 sm:py-10"><div className="mx-auto max-w-5xl px-4 sm:px-6">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,6fr)_minmax(18rem,4fr)]"><section className="rounded-xl border border-border bg-surface p-5 sm:p-7"><h2 className="text-lg font-bold">Ghế đã chọn</h2><ul className="mt-5 divide-y divide-border">{booking.items.map((item) => <li key={item.showtimeSeatId} className="flex items-center justify-between gap-4 py-3 text-sm"><span><span className="font-semibold">{item.seatLabel}</span> · {item.seatType === 'COUPLE' ? 'Ghế đôi' : item.seatType}</span><span>{money.format(item.unitPrice)}</span></li>)}</ul></section>
      <aside className="rounded-xl border border-border bg-surface p-5 lg:sticky lg:top-6 lg:h-fit"><h2 className="text-lg font-bold">Tổng thanh toán</h2><dl className="mt-5 space-y-3 text-sm"><div className="flex justify-between gap-3"><dt>Tạm tính</dt><dd>{money.format(booking.subtotal)}</dd></div>{booking.voucherCode && <div className="flex justify-between gap-3"><dt>Voucher</dt><dd>{booking.voucherCode}</dd></div>}<div className="flex justify-between gap-3"><dt>Giảm giá</dt><dd>{money.format(booking.discountAmount)}</dd></div><div className="flex justify-between gap-3"><dt>Phí dịch vụ</dt><dd>{money.format(booking.serviceFee)}</dd></div><div className="flex justify-between gap-3 border-t border-border pt-4 text-base font-bold"><dt>Tổng</dt><dd>{money.format(booking.totalAmount)}</dd></div></dl><p className="mt-5 text-sm text-text-secondary">Cổng thanh toán sẽ được mở ở bước tiếp theo sau khi hệ thống xác nhận yêu cầu.</p><button disabled className="mt-5 min-h-11 w-full rounded-lg bg-primary px-4 font-semibold text-white opacity-50">Thanh toán {money.format(booking.totalAmount)}</button></aside></div>
  </div></main>
}
